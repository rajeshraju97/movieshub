...
<div class="container" style="margin-top:7rem !important;">
    <h1 class="mb-4 text-center text-light">Airing Today Tv Series</h1>
...
<div class="d-flex justify-content-center">
    <nav>
        <ul class="pagination">
            @if ($currentPage > 1)
                <li class="page-item">
                    <a class="page-link" href="{{ url('tv/airing_today?page=' . ($currentPage - 1)) }}">Previous</a>
                </li>
            @endif
            ...
            @for ($i = $startPage; $i <= $endPage; $i++)
                <li class="page-item @if ($i == $currentPage) active @endif">
                    <a class="page-link" href="{{ url('tv/airing_today?page=' . $i) }}">{{ $i }}</a>
                </li>
            @endfor

            @if ($currentPage < $totalPages)
                <li class="page-item">
                    <a class="page-link" href="{{ url('tv/airing_today?page=' . ($currentPage + 1)) }}">Next</a>
                </li>
            @endif
        </ul>
    </nav>
...
<div class="d-flex justify-content-center mt-3">
    <form action="{{ url('tv/airing_today') }}" method="get" class="form-inline">
