...
<div class="container" style="margin-top:7rem !important;">
    <h1 class="mb-4 text-center text-light">Top Rated Movies</h1>
    ...
    <div class="d-flex justify-content-center">
        <nav>
            <ul class="pagination">
                @if ($currentPage > 1)
                    <li class="page-item">
                        <a class="page-link" href="{{ url('movies/trml?page=' . ($currentPage - 1)) }}">Previous</a>
                    </li>
                @endif
                ...
                @for ($i = $startPage; $i <= $endPage; $i++)
                    <li class="page-item @if ($i == $currentPage) active @endif">
                        <a class="page-link" href="{{ url('movies/trml?page=' . $i) }}">{{ $i }}</a>
                    </li>
                @endfor

                @if ($currentPage < $totalPages)
                    <li class="page-item">
                        <a class="page-link" href="{{ url('movies/trml?page=' . ($currentPage + 1)) }}">Next</a>
                    </li>
                @endif
            </ul>
        </nav>
    ...
    <div class="d-flex justify-content-center mt-3">
        <form action="{{ url('movies/trml') }}" method="get" class="form-inline">
