...
<div class="container" style="margin-top:7rem !important;">
    <div class="row">
                ...
                <form method="GET" action="{{ route('tv.series.list') }}">
                    <div class="mb-3">
                        ...
                        <select id="sort" name="sort" class="form-select" onchange="this.form.submit()">
                            <option value="popularity.desc" {{ $sort === 'popularity.desc' ? 'selected' : '' }}>Popularity Descending</option>
                            <option value="popularity.asc" {{ $sort === 'popularity.asc' ? 'selected' : '' }}>Popularity Ascending</option>
                            <option value="name.asc" {{ $sort === 'name.asc' ? 'selected' : '' }}>Title Ascending (A-Z)</option>
                            <option value="name.desc" {{ $sort === 'name.desc' ? 'selected' : '' }}>Title Descending (Z-A)</option>
                        </select>
                    </div>
                    <div class="mb-3">
        ...
        <!-- Main Content for TV Series Grid -->
        <div class="col-md-9">
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text">{{ Str::limit($tvShow['overview'], 100) }}</p>
                                <p class="card-text"><small class="text-muted">First Air Date: {{ $tvShow['first_air_date'] }}</small></p>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="d-flex justify-content-center">
                <nav>
                    <ul class="pagination">
                        @if ($currentPage > 1)
                            <li class="page-item">
                                <a class="page-link" href="{{ url('tv-series?page=' . ($currentPage - 1) . '&sort=' . $sort . '&language=' . $language) }}">Previous</a>
                            </li>
                        @endif
                        ...
                        @for ($i = $startPage; $i <= $endPage; $i++)
                            <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                <a class="page-link" href="{{ url('tv-series?page=' . $i . '&sort=' . $sort . '&language=' . $language) }}">{{ $i }}</a>
                            </li>
                        @endfor

                        @if ($currentPage < $totalPages)
                            <li class="page-item">
                                <a class="page-link" href="{{ url('tv-series?page=' . ($currentPage + 1) . '&sort=' . $sort . '&language=' . $language) }}">Next</a>
                            </li>
                        @endif
                    </ul>
                </nav>
